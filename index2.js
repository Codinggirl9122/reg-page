    ...
    const statesAndDistricts = [
        ...
        ["Arunachal Pradesh", ["Anjaw", "Changlang", "East Kameng", "East Siang", "Kamle", "Kra Daadi", "Kurung Kumey", "Lepa Rada", "Lohit", "Longding", "Lower Dibang Valley", "Lower Siang", "Lower Subansiri", "Namsai", "Pakke Kessang", "Papum Pare", "Shi Yomi", "Siang", "Tawang", "Tirap", "Upper Siang", "Upper Subansiri", "West Kameng", "West Siang"]],
        ...
        ["Jharkhand", ["Bokaro", "Dhanbad", "Deoghar", "Dumka", "East Singhbhum", "Garhwa", "Giridih", "Godda", "Gumla", "Hazaribagh", "Jamtara", "Khunti", "Koderma", "Latehar", "Lohardaga", "Pakur", "Palamu", "Ramgarh", "Ranchi", "Sahibganj", "Seraikela Kharsawan", "Simdega", "West Singhbhum"]],
    ...
    function updateDistricts() {
        ...
        const selectedStateData = statesAndDistricts.find(state => state[0] === selectedState);
        if (selectedStateData) {
            selectedStateData[1].forEach(district => {
    ...
    // Add initial state options
    const stateSelect = document.getElementById('state');
    statesAndDistricts.forEach(state => {
        const option = document.createElement('option');
    ...
    form.addEventListener('submit', function(event) {
        ...
        const cityVillage = document.getElementById('city-village').value.trim();
        ...
        const landlineNo = document.getElementById('landline-no').value.trim();
        const password = document.getElementById('password').value.trim();
        const confirmPassword = document.getElementById('confirm-password').value.trim();
        const terms = document.getElementById('terms').checked;
    
        // Validation checks
        if (!username || !email || !firstName || !lastName || !businessAddress || !cityVillage || !state || !district || !pinCode || !mobileNo || !password || !confirmPassword || !terms) {
        ...
        if (!/^(\+91[\-\s]?)?[0]?(91)?[789]\d{9}$/.test(mobileNo)) {
            alert('Invalid mobile number.');
            return;
        }
    
        if (landlineNo && !/^(\+91[\-\s]?)?[0]?(91)?[0-9]\d{9}$/.test(landlineNo)) {
